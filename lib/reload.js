...
module.exports = function reload (app, opts, server) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    const verboseLogging = opts.verbose === true || opts.verbose === 'true' || false
    const webSocketServerWaitStart = opts.webSocketServerWaitStart
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (socketPortSpecified) { // Use custom user specified port
          ...
          if (httpsOption) { // HTTPS
            if (fs.existsSync(httpsOption.key)) {
              httpsOptions.key = fs.readFileSync(httpsOption.key, 'utf-8')
            } else {
              console.log('Could not initialize reload could not read key file')
              return reject(new Error('Could not initialize reload could not read key file'))
            }

            if (fs.existsSync(httpsOption.cert)) {
              httpsOptions.cert = fs.readFileSync(httpsOption.cert, 'utf-8')
            } else {
              console.log('Could not initialize reload could not read cert file')
              return reject(new Error('Could not initialize reload could not read cert file'))
            }
    ...
    function getReloadReturn () {
      ...
      if (webSocketServerWaitStart) {
        tempObject.startWebSocketServer = function () {
