module.exports = function reload (app, opts, server) {
  ...
  if (arguments.length > 0 && (typeof (argumentZero) === 'object' || typeof (argumentZero) === 'function')) {
    if (typeof (argumentZero) === 'object') { // If old arguments passed handle old arguments, the old arguments and their order were: httpServerOrPort, expressApp, and verboseLogging
      ...
      if (arguments.length < 2) {
        throw new TypeError('Lack of/invalid arguments provided to reload. It is recommened to update to the new arguments anyways, this would be a good time to do so.', 'reload.js', 7)
      }
      ...
      if (route) {
         ...
         * 5. `newRoutePathreload.js` -> (Strip reload.js above) so now: Thier route + `/` + reload.js
         * 6. `newRoutePath/reload.js/rEload.js/... reload.js n number of times -> (Strip above removes all reload.js occurences at the end of the specified route) so now: Their route + 'reload.js`
        */
  ...
  // Return an object, right now it contains a function reload. With this function is called it calls the sendMessage function with the message 'reload' which reloads all connected clients.
  return {
    'reload': function () {
      sendMessage('reload')
    }
