...
/** Returns promise of Object{httpServer, reload, port}
...
/** returns Function(request, response) . Does not watch files, use reloadSocketByHttp to establish watch and websocket
  @pathTo String - path to serve and if options.watch also folder to watch
*/
function middleware(pathTo){
  ...
  var routes = function(req, res){
    ...
    var index = req.url ==='/' || req.url.search(/[^?]*(\.html)(\?.)*/)>=0
...
function createFolderWatchServer(outputFileFolder, options){
  ...
  var app = module.exports.middleware(outputFileFolder)
