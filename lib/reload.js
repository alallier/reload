...
module.exports = function reload (app, opts, server) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    function setupClientSideCode () {
      ...
      if (forceWss) {
        reloadCode = reloadCode.replace('const forceWss = false', 'const forceWss = true')
      }

      if (socketPortSpecified) {
        reloadCode = reloadCode.replace('const port = window.location.port', `const port = ${socketPortSpecified}`)
      }
    }
