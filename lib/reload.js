...
module.exports = function reload (httpServerOrPort, expressApp, verboseLogging) {
  var socketPortSpecified = typeof httpServerOrPort === 'number' ? httpServerOrPort : null
  ...
  if (socketPortSpecified) {
    reloadCode = reloadCode.replace('window.location.origin.replace()', 'window.location.origin.replace(/(^http(s?):\\/\\/)(.*:)(.*)/, \'ws$2://$3' + httpServerOrPort + '\')')
  } else {
