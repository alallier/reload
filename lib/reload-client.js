...
(function refresh () {
  ...
  const port = window.location.port // This line can be overridden by reload server, depending on configuration
  const forceWss = false // This line can be overridden by reload server, depending on configuration

  const wsProtocol = url.protocol === 'https:' || forceWss === true ? 'wss:' : 'ws:'

  const socketUrl = `${wsProtocol}//${url.hostname}${port ? `:${port}` : ''}`

  let socket

  if (verboseLogging) {
    console.log('Reload Script Loaded')
    console.log('Reload socket URL: ', socketUrl)
  }
